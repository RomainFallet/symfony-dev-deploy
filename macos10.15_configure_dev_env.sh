...
# Install
if ! /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)"; then exit 1; fi
if ! brew -v; then exit 1; fi

# Install
brew install git
if ! brew install git; then exit 1; fi
# Reload $PATH
if ! export PATH="/usr/local/bin:$PATH"; then exit 1; fi
...
# Install
if ! brew install php@7.3; then exit 1; fi

# Replace default macOS PHP installation in $PATH
if ! brew link php@7.3 --force; then exit 1; fi

# Reload $PATH
if ! export PATH="/usr/local/opt/php@7.3/bin:$PATH"; then exit 1; fi

# Install extensions
if ! pecl install xdebug; then exit 1; fi
...
# Install
if ! brew install mariadb@10.4; then exit 1; fi
if ! brew services start mariadb; then exit 1; fi
if ! sudo mysql -e "SELECT VERSION();"; then exit 1; fi

# Install
if ! brew install node@12; then exit 1; fi
# Add node to $PATH
if ! brew link node@12 --force; then exit 1; fi
if ! node -v; then exit 1; fi
if ! npm -v; then exit 1; fi

# Install
if ! curl -o- -L https://yarnpkg.com/install.sh | bash -s -- --version 1.21.1; then exit 1; fi

# Reload $PATH
if ! export PATH="$HOME/.yarn/bin:$HOME/.config/yarn/global/node_modules/.bin:$PATH"; then exit 1; fi
